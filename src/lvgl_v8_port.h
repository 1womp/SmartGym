 ...
 */
#define LVGL_PORT_BUFFER_MALLOC_CAPS            (MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)      // Keep internal SRAM free for Wi-Fi/TLS
...
#define LVGL_PORT_TASK_STACK_SIZE               (24 * 1024) // Single-context recovery mode may run compact cloud queue work from the LVGL task, so the stack must be larger
